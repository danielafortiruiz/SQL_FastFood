...
-- POBLAR tablas mediante queries:
...
SELECT * FROM Sucursales  -- VERIFICAMOS QUE HAYA SIDO MODIFICADA 
...
SELECT * FROM Ordenes  -- VEMOS QUE LA OrdenID 10 NO se ve mas en la tabla 
...
DELETE FROM Empleados WHERE SucursalID = 10;  -- NO elimina ninguno por que no existe sucursalID 10. 


--CONSULTA 1 ¿Cual es la cantidad total de registros unicos en la tabla de ordenes? 
use FastfoodDB
...
-- RESPUESTA: la cantidad de registros unicos = 9


-- CONSULTA 2 ¿Cuantos empleados existen en cada departamaneto? 
SELECT * FROM Empleados

SELECT count (EmpleadoID) AS Cantidad 
FROM Empleados
GROUP BY Departamento
ORDER BY Cantidad DESC; 


-- CONSULTA 3: ¿Cuantos productos hay por codigo de categoria? 
SELECT * FROM Productos
...
SELECT Count (ClienteID) Conteo 
FROM Clientes;

/* -- CONSULTA 5: ¿Cuales son las sucursales con un promedio de ventas por orden 
superior a un valor especifico (40), 
ordenadas por el promedio de KM recorridos para las entregas de mayor a menor ? */ 

SELECT * FROM Ordenes

SELECT SucursalId,
AVG (TotalCompra) AS Promedio_venta,
AVG (KilometrosRecorrer) AS Promedio_kilometros
FROM Ordenes
GROUP BY SucursalID
